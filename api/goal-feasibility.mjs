...
function parseMinutes(value){
  ...
  m=s.match(/(?:(\d+)\s*h)?\s*(?:(\d+)\s*(?:min|m))?/i);if(m&&(m[1]||m[2]))return Number(m[1]||0)*60+Number(m[2]||0);
  return null;
}
function parseDistanceKm(value){if(value==null)return null;const m=String(value).replace(',','.').match(/\d+(?:\.\d+)?/);return m?Number(m[0]):null;}
...
function thresholdEstimate(pace,distance){
  if(!pace||!distance)return null;let adj;if(distance<=5)adj=-15;else if(distance<=10)adj=-5;else if(distance<=HALF_KM)adj=10;else if(distance<=42.3)adj=30;else adj=45;return distance*(pace+adj)/60;
}
function preparationPotential(weeks,sessions,conservative){
  let p=weeks>=16?.04:weeks>=12?.035:weeks>=9?.025:weeks>=6?.015:weeks>=4?.0075:0;
  if(sessions<=2)p*=.55;else if(sessions>=5)p*=1.1;if(conservative)p*=.45;return clamp(p,0,.045);
}
function roundMinute(v){return Math.max(1,Math.round(v));}
function sourceEstimate(state,objective){
  const live=state?.meta?.corosMode==='mcp';const metrics=live?(state.metrics||{}):{};const d=objective.distanceKm;const estimates=[];
  ...
  if(!estimates.length){
    const a=state.latestActivity||null,ad=parseDistanceKm(a?.distance),at=parseMinutes(a?.duration);
    if(ad&&at&&ad>=3&&d)estimates.push({name:'dernière activité',minutes:riegel(at,ad,d)*.96,weight:.22,weak:true});
  }
...
export function assessGoalFeasibility(state={},objective={}){
  const today=state?.meta?.today||new Date().toISOString().slice(0,10),start=objective.startDate||today,end=objective.date||null,d=objective.distanceKm||null,target=objective.targetDurationMinutes||parseMinutes(objective.target),weeks=end?Math.max(0,Math.ceil(daysBetween(start,end)/7)):0,sessions=Number(objective.sessionsPerWeek||4);
  ...
  const estimate=sourceEstimate(state,objective),current=estimate.minutes,potential=preparationPotential(weeks,sessions,conservative),prepared=current?current*(1-potential):null;
  ...
  const ratioTarget=target/prepared;let status,label,decision;
  if(ratioTarget>=1.04){status='comfortable';label='Prudent / réaliste';decision='go';}
  else if(ratioTarget>=.98){status='realistic';label='Réaliste';decision='go';}
  else if(ratioTarget>=.93){status='ambitious';label='Ambitieux';decision='caution';}
  else{status='too-aggressive';label='Trop agressif actuellement';decision='adjust';}
  const recommended=roundMinute(prepared),range=[roundMinute(prepared*.98),roundMinute(prepared*1.05)];
  const delta=Math.round((1-ratioTarget)*100);if(status==='ambitious')reasons.push(`Le chrono demandé est environ ${Math.max(1,delta)} % plus rapide que l’estimation préparée.`);if(status==='too-aggressive')reasons.push(`Le chrono demandé est environ ${Math.max(1,delta)} % plus rapide que ce que les données actuelles rendent raisonnable.`);
