(() => {
  ...
  function mergeRecords(server=[],local=[],identity=item=>item?.id||''){
    ...
    for(const item of local||[]){const id=identity(item);if(id)map.set(id,item);}
    return [...map.values()];
  }
  function overlay(data){
    const local=read();if(!local.savedAt)return data;
    const deleted=new Set(local.deletedObjectiveIds||[]),objectives=(data.objectives||[]).filter(o=>!deleted.has(o.id)),plans=(data.plans||[]).filter(p=>!deleted.has(p.objectiveId));
    for(const o of local.objectives||[]){if(deleted.has(o.id))continue;const i=objectives.findIndex(x=>x.id===o.id);if(i>=0)objectives[i]=o;else objectives.unshift(o);}
    for(const p of local.plans||[]){if(deleted.has(p.objectiveId))continue;const i=plans.findIndex(x=>x.id===p.id);if(i>=0)plans[i]=p;else plans.unshift(p);}
    const feedback=mergeRecords(data.feedback||[],local.feedback||[],item=>item?.id||`${item?.at||''}:${item?.sessionId||item?.activityId||''}`).sort((a,b)=>String(b.at||'').localeCompare(String(a.at||'')));
    const coachMessages=mergeRecords(data.coachMessages||[],local.coachMessages||[],item=>`${item?.at||''}:${item?.text||''}`).sort((a,b)=>String(b.at||'').localeCompare(String(a.at||'')));
    const activeObjective=objectives.find(o=>o.status==='active')||null,activePlan=activeObjective?plans.find(p=>p.id===activeObjective.planId)||null:null,today=data.meta?.today||new Date().toISOString().slice(0,10);
    return{...data,objectives,plans,feedback,coachMessages,planVersions:local.planVersions?.length?local.planVersions:(data.planVersions||[]),planProposals:local.planProposals?.length?local.planProposals:(data.planProposals||[]),activeObjective,activePlan,todaySession:activePlan?.sessions?.find(s=>s.date===today)||null,meta:{...(data.meta||{}),deviceContinuity:true}};
  }
  async function capture(data=null){
    try{
      if(!data){const r=await originalFetch('/api/dashboard',{cache:'no-store'});if(!r.ok)return;data=await r.json();}
      const prev=read(),deleted=new Set(prev.deletedObjectiveIds||[]);
      write({deletedObjectiveIds:[...deleted],objectives:(data.objectives||[]).filter(o=>!deleted.has(o.id)),plans:(data.plans||[]).filter(p=>!deleted.has(p.objectiveId)),planVersions:data.planVersions||prev.planVersions||[],planProposals:(data.planProposals||[]).filter(p=>p.status!=='proposed'),feedback:data.feedback||prev.feedback||[],coachMessages:data.coachMessages||prev.coachMessages||[]});
